...
use App\Domain\Repositories\Pelayanan\SatuOrangRepository;
...
class CetakLaporanSatuOrang extends Controller
{
    protected $crud;
    protected $paginate;
//330 210
    ...
    public function __construct(
        SatuOrangRepository $satuOrangRepository,
    ...
    {
        $this->satuorang = $satuOrangRepository;
    ...
    function Cover($pdf, $id)
    {
        ...
        $kodeadministrasikearsipan = $this->satuorang->cekkodejenisadministrasicetak();
        //kabupaten
        ...
        $pdf->Ln(10);
        $pdf->Cell(0, 5, 'SURAT KETERANGAN SATU ORANG', 0, 0, 'C');
        $pdf->Ln(10);
...
//        echo $space_bottom . ' + ';
    ...
    function Column($pdf, $id)
    {
        ...
        $pdf->Cell(45, 20, 'Jenis Perbedaan Data', 1, 0, 'C');
        $pdf->Cell(45, 20, 'Data Seharusnya', 1, 0, 'C');
    ...
    function Column2($pdf)
    {
        ...
        $pdf->Ln(10);
        $pdf->Cell(0, 10, 'SURAT KETERANGAN SATU ORANG', 0, 0, 'C');
        $pdf->Ln(10);
    }

    public function LaporanSatuOrang($id)
    {
//        array(215, 330)
        ...
        $this->Column2($pdf);
        $this->Column($pdf, $id);
        $jumlah = $this->satuorang->getByPagecetak($id);
        ...
        $pdf->SetWidths([25, 20, 40, 20, 70, 45, 45, 30, 30, 30, 30]);
        ...
        if ($jumlah == null) {
            ...
            foreach ($jumlah as $row) {
                ...
                $pdf->Row([$row->tanggal, $row->no_reg, $row->nik . $pribadi->titel_depan . ' ' . $pribadi->nama . ' ' . $pribadi->titel_belakang, $jk->jk, $keluarga->alamat . '|' . ' RT ' . $keluarga->alamat_rt . '| RW ' . $keluarga->alamat_rw . ' ' . $statusdesa . ' ' . $namadesa . ' ' . $statuskecamatan . ' ' . $kecamatan . ' ' . $status . ' ' . $kabupaten, $row->beda_data1 . ' | ' . $row->beda_data2 . ' | ' . $row->beda_data3, $row->data_seharusnya1 . ' | ' . $row->data_seharusnya2 . ' | ' . $row->data_seharusnya3, $row->penggunaan, $namapejabatna, $users->name, '']);
...
//                $this->butuh = true;
            ...
            $tanggal = $this->satuorang->getByPagelimittanggal($id);
            if ($tanggal != null) {
