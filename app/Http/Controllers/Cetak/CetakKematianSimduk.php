...
class CetakKematianSimduk extends Controller
{
    protected $crud;
    protected $paginate;
    protected $akun;
...
//330 210
    ...
    function Kop($pdf, $id)
    {
        ...
        if ($Kematian->is_penduduk_layan != null) {
            $pdf->Cell(0, 0, 'SURAT KETERANGAN KEMATIAN' . ' ' . strtoupper($Kematian->is_penduduk_layan), 0, '', 'C');
...
//        echo $space_bottom . ' + ';
    ...
    public function KematianSimduk($id)
    {
//        array(215, 330)
        ...
        $pdf->SetTitle('Surat Kematian Simduk');
        $this->Kop($pdf, $id);
        $pdf->SetY(50);
        $desa = $this->desa->find(session('desa'));
        $Kematian = $this->Kematian->find($id);
...
//tanggal lahir
        ...
        $pdf->SetFont('Arial', '', 8);
        $pdf->SetX(14);
        $pdf->Cell(5, -15, 'dengan ini menerangkan bahwa:', 0, '', 'L');
...
//        pelapor bayi
        ...
        $pdf->Ln(10);
        $pdf->SetX(14);
        $pdf->Cell(25, -15, 'Hubungan pelapor dengan yang mati : '.$Kematian->shdrt->shdrt, 5, '', 'L');
