...
//330 210
    ...
    function Cover($pdf, $id)
    {
        ...
        $kodeadministrasikearsipan = $this->multifungsi->cekkodejenisadministrasicetak();
        //kabupaten
        ...
        $pdf->Ln(10);
        $pdf->Cell(0, 5, 'SURAT MULTIFUNGSI', 0, 0, 'C');
        $pdf->Ln(10);
    ...
    function Column2($pdf)
    {
        ...
        $pdf->Ln(10);
        $pdf->Cell(0, 10, 'SURAT MULTIFUNGSI', 0, 0, 'C');
        $pdf->Ln(10);
    }
...
//        array(215, 330)
        ...
        $this->Cover($pdf, $id);
        $pdf->AddPage();
        $pdf->SetTitle('Laporan Register Surat Multifungsi');
        ...
        $this->Column2($pdf);
        $this->Column($pdf, $id);
        $jumlah = $this->multifungsi->getByPagecetak($id);
        ...
        $pdf->SetAligns(['C', 'C', 'C', 'C', 'JC', 'JC', 'JC', 'C', 'C']);
        $pdf->SetWidths([25, 20, 40, 20, 70, 70, 30, 30, 30, 30]);
        ...
        if ($jumlah == null) {
            ...
            foreach ($jumlah as $row) {
                ...
                $this->butuh = true;
                if ($row->redaksi_2 == 1 && $row->redaksi_3 == 1) {
                    $redaksi = $row->redaksi_1;
                }
                if ($row->redaksi_2 == 1 && $row->redaksi_3 != 1) {
                    $redaksi = '1. ' . $row->redaksi_1 . ' 2. ' . $row->redaksi_2;
                }
                if ($row->redaksi_2 != 1 && $row->redaksi_3 != 1) {
                    $redaksi = '1. ' . $row->redaksi_1 . ' 2. ' . $row->redaksi_2 . ' 3. ' . $row->redaksi_3;
                }

                if ($row->is_jenis_layan == 1) {
                    $keluarga = $this->keluarga->cekalamat($row->penduduk_id);
                    $pribadi = $this->pribadi->find($row->penduduk_id);

                    $jk = $this->jk->find($pribadi->jk_id);
                    $pdf->Row([$row->tgl, $row->no_reg, $row->nik_penduduk . $pribadi->titel_depan . ' ' . $pribadi->nama . ' ' . $pribadi->titel_belakang, $jk->jk, $keluarga->alamat . '' . ' RT ' . $keluarga->alamat_rt . ' RW ' . $keluarga->alamat_rw . ' ' . $statusdesa . ' ' . $namadesa . ' ' . $statuskecamatan . ' ' . $kecamatan . ' ' . $status . ' ' . $kabupaten, $redaksi, $row->penggunaan_surat, $namapejabatna, $users->name, '']);

                }
                if ($row->is_jenis_layan == 2) {
                    ...
                    $pdf->Row([$row->tgl, $row->no_reg, $row->nik_penduduk . $nonpenduduk->titel_depan . ' ' . $nonpenduduk->nama . ' ' . $nonpenduduk->titel_belakang, $jk->jk, $nonpenduduk->alamat . '|' . ' RT ' . $nonpenduduk->alamat_rt . '| RW ' . $nonpenduduk->alamat_rw . ' ' . $statusdesanonpenduduk . ' ' . $namadesanonpenduduk . ' ' . $statuskecamatannonpenduduk . ' ' . $kecamatannonpenduduk . ' ' . $statuskabupatennonpenduduk . ' ' . $kabupatennonpenduduk, $redaksi, $row->penggunaan_surat, $namapejabatna, $users->name, '']);
...
//                $this->butuh = true;
            ...
            $tanggal = $this->multifungsi->getByPagelimittanggal($id);
            if ($tanggal != null) {
                ...
                if ($pejabatsekretaris != null) {
                    ...
                    if ($tanggal != null) {
                        ...
                        if (substr($tanggal->tanggal, 3, 2) <= 9) {
                            $bulan3 = $indo3[substr($tanggal->tgl, 4, 1)];
                        } else {
                            $bulan3 = $indo3[substr($tanggal->tgl, 3, 2)];
                        }
                        $tahun3 = substr($tanggal->tgl, 6, 4);
