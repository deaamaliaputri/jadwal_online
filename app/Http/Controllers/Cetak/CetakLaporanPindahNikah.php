...
class CetakLaporanPindahNikah extends Controller
{
    protected $crud;
    protected $paginate;
//330 210
    ...
    function Cover($pdf, $id)
    {
        ...
        $kodeadministrasikearsipan = $this->pindahnikah->cekkodejenisadministrasicetak();
        //kabupaten
        ...
        $pdf->Ln(10);
        $pdf->Cell(0, 5, 'SURAT KETERANGAN PINDAH NIKAH', 0, 0, 'C');
        $pdf->Ln(10);
...
//        echo $space_bottom . ' + ';
    ...
    function Column($pdf, $id)
    {
        ...
        $pdf->Cell(70, 20, 'Alamat Asal', 1, 0, 'C');
        $pdf->Cell(70, 20, 'Alamat Tujuan', 1, 0, 'C');
    ...
    function Column2($pdf)
    {
        ...
        $pdf->Ln(10);
        $pdf->Cell(0, 10, 'SURAT KETERANGAN PINDAH NIKAH', 0, 0, 'C');
        $pdf->Ln(10);
    }

    public function LaporanPindahNikah($id)
    {
//        array(215, 330)
        ...
        $this->Cover($pdf, $id);
        $pdf->AddPage();
        $pdf->SetTitle('Laporan Register Surat Pindah Nikah');
        ...
        $this->Column2($pdf);
        $this->Column($pdf, $id);
        $jumlah = $this->pindahnikah->getByPagecetak($id);
        ...
        if ($jumlah == null) {
            ...
            foreach ($jumlah as $row) {
                ...
                $pdf->Row([$row->tanggal, $row->no_reg, $row->nik . $pribadi->titel_depan . ' ' . $pribadi->nama . ' ' . $pribadi->titel_belakang, $jk->jk, $keluarga->alamat . '|' . ' RT ' . $keluarga->alamat_rt . '| RW ' . $keluarga->alamat_rw . ' ' . $statusdesa . ' ' . $namadesa . ' ' . $statuskecamatan . ' ' . $kecamatan . ' ' . $status . ' ' . $kabupaten,  $row->alamat . ' | RT. ' . $row->alamat_rt . ' | RW. ' . $row->alamat_rw . ' ' . $statusdesatujuan . ' ' . $namadesatujuan . ' ' . $statuskecamatantujuan . ' ' . $kecamatantujuan . ' ' . $statustujuan . ' ' . $kabupatentujuan, $row->penggunaan_surat, $namapejabatna, $users->name, '']);
...
//                $this->butuh = true;
            ...
            $tanggal = $this->pindahnikah->getByPagelimittanggal($id);
            if ($tanggal != null) {
