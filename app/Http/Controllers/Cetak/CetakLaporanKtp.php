...
class CetakLaporanKtp extends Controller
{
    protected $crud;
    protected $paginate;
//330 210
    ...
    function Cover($pdf, $id)
    {
        ...
        $kodeadministrasikearsipan = $this->ktp->cekkodejenisadministrasicetak();
        //kabupaten
        ...
        $pdf->Ln(10);
        $pdf->Cell(0, 5, 'PERMOHONAN KARTU TANDA PENDUDUK', 0, 0, 'C');
        $pdf->Ln(10);
...
//        echo $space_bottom . ' + ';
    ...
    function Column($pdf, $id)
    {
        ...
        $pdf->Ln(10);
        $pdf->Cell(30, 20, 'Tanggal', 1, 0, 'C');
        $pdf->Cell(30, 19, 'Nomor', 'TLR', 0, 'C');
        $pdf->Cell(40, 20, 'NIK', 'TLR', 0, 'C');
        $pdf->Cell(30, 20, 'Jenis Kelamin', 1, 0, 'C');
        $pdf->Cell(70, 20, 'Nomor Kartu Keluarga', 1, 0, 'C');
        $pdf->Cell(45, 20, 'Jenis Permohonan', 1, 0, 'C');
        $pdf->Cell(45, 20, 'Penandatangan', 1, 0, 'C');
        $pdf->Cell(45, 20, 'Operator', 1, 0, 'C');
        $pdf->Cell(30, 20, 'Keterangan', 1, 0, 'C');
        $pdf->Ln(10);
        $pdf->Cell(30);
        $pdf->Cell(30, 10, 'Register', 'BLR', 0, 'C');
        $pdf->Cell(40, 10, 'Nama Pemohon', 'BLR', 0, 'C');
        $pdf->Cell(30);
        $pdf->Cell(70, 10, 'Alamat Domisili', 'BLR', 0, 'C');
        $pdf->Ln(10);
    ...
    function Column2($pdf)
    {
        ...
        $pdf->Ln(10);
        $pdf->Cell(0, 10, 'PERMOHONAN KARTU TANDA PENDUDUK', 0, 0, 'C');
        $pdf->Ln(10);
    }

    public function Laporanktp($id)
    {
        ...
        $this->Cover($pdf, $id);
        $pdf->AddPage();
        $pdf->SetTitle('Laporan Permohonan Kartu Tanda Penduduk');
        ...
        $this->Column2($pdf);
        $this->Column($pdf, $id);
        $jumlah = $this->ktp->getByPagecetak($id);
        $desa = $this->desa->find(session('desa'));
        ...
        $pdf->SetWidths([30, 30, 40, 30, 70, 45, 45, 45, 30, 45, 30]);
        ...
        if ($jumlah == null) {
            ...
            foreach ($jumlah as $row) {
                ...
                if ($row->pejabat_desa_id == 1) {
                    ...
                    $alamatlengkap = ' (' . $keluarga->alamat . '' . ' RT ' . $keluarga->alamat_rt . ' RW ' . $keluarga->alamat_rw . ' ' . $statusdesa . ' ' . $namadesa . ' ' . $statuskecamatan . ' ' . $kecamatan . ' ' . $status . ' ' . $kabupaten . ' )';
                    $pdf->Row([$row->tanggal, $row->no_reg, $row->nik . $pribadi->titel_depan . ' ' . $pribadi->nama . ' ' . $pribadi->titel_belakang, $jk->jk, $keluarga->nomor_kk . $alamatlengkap, $row->is_jenis_ktp, $namapejabatna, $users->name, '']);

                }
                if ($row->pejabat_desa_id == 2) {
                    $nonpenduduk = $this->nonpenduduk->find($row->penduduk_id);
                    $rinciannon = $this->rinciannonpenduduk->ceknomorkk($row->penduduk_id);
                    ...
                    $alamatlengkap = ' (' . $nonpenduduk->alamat . '|' . ' RT ' . $nonpenduduk->alamat_rt . '| RW ' . $nonpenduduk->alamat_rw . ' ' . $statusdesanonpenduduk . ' ' . $namadesanonpenduduk . ' ' . $statuskecamatannonpenduduk . ' ' . $kecamatannonpenduduk . ' ' . $statuskabupatennonpenduduk . ' ' . $kabupatennonpenduduk . ' )';
                    $pdf->Row([$row->tanggal, $row->no_reg, $row->nik . $nonpenduduk->titel_depan . ' ' . $nonpenduduk->nama . ' ' . $nonpenduduk->titel_belakang, $jk->jk, $rinciannon->rincian_non_penduduk . $alamatlengkap, $row->is_jenis_ktp, $namapejabatna, $users->name, '']);
...
//                $this->butuh = true;
            ...
            $tanggal = $this->ktp->getByPagelimittanggal($id);
            if ($tanggal != null) {
