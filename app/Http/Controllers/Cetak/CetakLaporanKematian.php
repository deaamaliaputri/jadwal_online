...
class CetakLaporanKematian extends Controller
{
    protected $crud;
    protected $paginate;
//330 210
    ...
    function Cover($pdf, $id)
    {
        ...
        $kodeadministrasikearsipan = $this->Kematian->cekkodejenisadministrasicetak();
        //kabupaten
        ...
        $pdf->Ln(10);
        $pdf->Cell(0, 5, 'SURAT KETERANGAN KEMATIAN', 0, 0, 'C');
        $pdf->Ln(10);
...
//        echo $space_bottom . ' + ';
    ...
    function Column($pdf, $id)
    {
        ...
        $pdf->Cell(60, 20, 'Alamat', 'TLR', 0, 'C');
        $pdf->Cell(40, 20, 'Tanggal Mati/', 'TLR', 0, 'C');
        $pdf->Cell(30, 20, 'Waktu', 'TLR', 0, 'C');
        $pdf->Cell(40, 20, 'Sebab', 'TLR', 0, 'C');
        ...
        $pdf->Cell(40, 10, 'Nama Jenazah', 'BLR', 0, 'C');
        $pdf->Cell(85);
        $pdf->Cell(40, 10, 'Umur', 'BLR', 0, 'C');
        $pdf->Cell(30, 10, 'Kematian', 'BLR', 0, 'C');
        $pdf->Cell(40, 10, 'Kematian', 'BLR', 0, 'C');
        $pdf->Ln(10);
    ...
    function Column2($pdf)
    {
        ...
        $pdf->Ln(10);
        $pdf->Cell(0, 10, 'SURAT KETERANGAN KEMATIAN', 0, 0, 'C');
        $pdf->Ln(10);
    }
...
//        array(215, 330)
        ...
        $this->Cover($pdf, $id);
        $pdf->AddPage();
        $pdf->SetTitle('Laporan Register Surat Kematian');
        ...
        $this->Column2($pdf);
        $this->Column($pdf, $id);
        $jumlah = $this->Kematian->getByPagecetak($id);
        ...
        $pdf->SetWidths([25, 25, 40, 25, 60, 40, 30, 40, 35, 30, 40]);
        ...
        if ($jumlah == null) {
            ...
            foreach ($jumlah as $row) {
                ...
                if ($row->jenis_penduduk == 1) {
                    ...
                    $tahun2 = substr($row->pribadi->tanggal_lahir, 6, 4);
                }
                if ($row->jenis_penduduk == 2) {
                    $namajenazah = $row->non_penduduk->nama;
                    ...
                    $tahun2 = substr($row->non_penduduk->tanggal_lahir, 6, 4);
                ...
                $hari = substr($row->tanggal_kematian, 0, 2);
                $indo = array("", "Januari", "Februari", "Maret", "April", "Mei", "Juni", "Juli", "Agustus", "September", "Oktober", "November", "Desember");
                if (substr($row->tanggal_kematian, 3, 2) <= 9) {
                    $bulan = $indo[substr($row->tanggal_kematian, 4, 1)];
                } else {
                    $bulan = $indo[substr($row->tanggal_kematian, 3, 2)];
                }
                $tahun = substr($row->tanggal_kematian, 6, 4);

                $tanggal_kematiancetak = $hari . ' ' . $bulan . ' ' . $tahun;

                $umur =$tahun -$tahun2 .' Tahun';
                $pdf->Row([$row->tanggal, $row->no_reg, $row->nik . $namajenazah, $jk, $alamatregional, $tanggal_kematiancetak.'/'.$umur, $row->waktu_mati,$row->sebab_kematian,$namapejabatna , $users->name, '']);
...
//                $this->butuh = true;
            ...
            $tanggal = $this->Kematian->getByPagelimittanggal($id);
            if ($tanggal != null) {
