...
class CetakLaporanKeramaian extends Controller
{
    protected $crud;
    protected $paginate;
//330 210
    ...
    function Cover($pdf, $id)
    {
        ...
        $kodeadministrasikearsipan = $this->Keramaian->cekkodejenisadministrasicetak();
        //kabupaten
        ...
        $pdf->Ln(10);
        $pdf->Cell(0, 5, 'SURAT KERAMAIAN', 0, 0, 'C');
        $pdf->Ln(10);
...
//        echo $space_bottom . ' + ';
    ...
    function Column2($pdf)
    {
        ...
        $pdf->Ln(10);
        $pdf->Cell(0, 10, 'SURAT KERAMAIAN', 0, 0, 'C');
        $pdf->Ln(10);
    }

    public function LaporanKeramaian($id)
    {
//        array(215, 330)
        ...
        $this->Cover($pdf, $id);
        $pdf->AddPage();
        $pdf->SetTitle('Laporan Register Surat Keramaian');
        ...
        $this->Column2($pdf);
        $this->Column($pdf, $id);
        $jumlah = $this->Keramaian->getByPagecetak($id);
        $desa1 = $this->desa->find(session('desa'));
        ...
        if ($jumlah == null) {
            ...
            foreach ($jumlah as $row) {
                ...
                if ($row->jenis_penduduk == 2) {
                    $non_penduduk = $this->nonpenduduk->find($row->penduduk_id);

                    $desa = $this->desa->find($non_penduduk->desa_id);
                }
                if ($desa->kecamatan->kabupaten->status == 1) {
                    $status = 'Kabupaten';
                    $kabupaten = $desa->kecamatan->kabupaten->kabupaten;
                ...
                if ($row->jenis_ijin == 'Ijin Lainnya') {
                    $jenisijin = $row->jenis_ijin_lainnya;
                } else {
                    $jenisijin = $row->jenis_ijin;
                }
                if ($row->jenis_kegiatan == 'Lainnya') {
                    $jeniskegiatan = $row->jenis_kegiatan_lainnya;
                } else {
                    $jeniskegiatan = $row->jenis_kegiatan;
                }
                if ($row->jenis_hiburan == 'Lainnya') {
                    $jenishiburan = $row->jenis_hiburan_lainnya;
                } else {
                    $jenishiburan = $row->jenis_hiburan;
                }
                if ($row->jenis_penduduk == 1) {
                    ...
                    $pdf->Row([$row->tanggal, $row->no_reg, $row->nik . $pribadi->titel_depan . ' ' . $pribadi->nama . ' ' . $pribadi->titel_belakang, $jk->jk, $keluarga->alamat . '|' . ' RT ' . $keluarga->alamat_rt . '| RW ' . $keluarga->alamat_rw . ' ' . $statusdesa . ' ' . $namadesa . ' ' . $statuskecamatan . ' ' . $kecamatan . ' ' . $status . ' ' . $kabupaten, $jenisijin, $jeniskegiatan, $jenishiburan, $namapejabatna, $users->name, '']);
                }
                if ($row->jenis_penduduk == 2) {

                    $jk = $this->jk->find($non_penduduk->jk_id);
                    $desanonpenduduk = $this->desa->find($row->non_penduduk->desa_id);
                    //kabupaten
                    ...
                    $alamatlengkap =$row->non_penduduk->alamat . '' . ' RT. ' . $row->non_penduduk->alamat_rt . ' RW. ' . $row->non_penduduk->alamat_rw . ' ' . $statusdesanonpenduduk . ' ' . $namadesanonpenduduk . ' ' . $statuskecamatannonpenduduk . ' ' . $kecamatannonpenduduk . ' ' . $statuskabupatennonpenduduk . ' ' . $kabupatennonpenduduk;

                    $pdf->Row([$row->tanggal, $row->no_reg, $row->nik . $non_penduduk->titel_depan . ' ' . $non_penduduk->nama . ' ' . $non_penduduk->titel_belakang, $jk->jk, $non_penduduk->alamat . '|' . ' RT ' . $non_penduduk->alamat_rt . '| RW ' . $non_penduduk->alamat_rw . ' ' . $statusdesa . ' ' . $alamatlengkap, $jenisijin, $jeniskegiatan, $jenishiburan, $namapejabatna, $users->name, '']);
                }
//                $this->butuh = true;
                $organisasi = $this->organisasi->find(session('organisasi'));
            ...
            $tanggal = $this->Keramaian->getByPagelimittanggal($id);
            if ($tanggal != null) {
                ...
                if ($pejabatsekretaris != null) {
                    ...
                    if ($tanggal != null) {
                        ...
                        $pdf->Row2(['', $namadesa1 . ', ' . $tempatlahir3]);
                    }
                    if ($pejabatsekretaris != null) {
                        ...
                        $pdf->Row2(['', $keteraganjabatan5 . strtoupper($pejabatsekretaris->jabatan . ' ' . $namadesa1 . ',')]);
                    }
...
//        $pdf->set_line = false;
        ...
        $pdf->Output('cetak-data-register-keramaian' . $tanggal . '.pdf', 'I');
        exit;
    }
}