...
class CetakLaporanKelahiran extends Controller
{
    protected $crud;
    protected $paginate;
//330 210
    ...
    function Cover($pdf, $id)
    {
        ...
        $kodeadministrasikearsipan = $this->Kelahiran->cekkodejenisadministrasicetak();
        //kabupaten
        ...
        $pdf->Ln(10);
        $pdf->Cell(0, 5, 'SURAT KETERANGAN KELAHIRAN', 0, 0, 'C');
        $pdf->Ln(10);
...
//        echo $space_bottom . ' + ';
    ...
    function Column($pdf, $id)
    {
        ...
        $pdf->Cell(25, 19, 'Nomor', 'TLR', 0, 'C');
        $pdf->Cell(50, 17, 'Nama Bayi', 'TLR', 0, 'C');
        $pdf->Cell(25, 20, 'L/P ', 1, 0, 'C');
        $pdf->Cell(40, 20, 'Nama Lengkap', 'TLR', 0, 'C');
        $pdf->Cell(40, 20, 'Nama Lengkap', 'TLR', 0, 'C');
        $pdf->Cell(70, 20, 'Alamat Domisili', 1, 0, 'C');
        $pdf->Cell(35, 20, 'Penandatangan', 1, 0, 'C');
        $pdf->Cell(30, 20, 'Operator', 1, 0, 'C');
        $pdf->Cell(40, 20, 'Keterangan', 1, 0, 'C');
        $pdf->Ln(10);
        $pdf->Cell(25);
        $pdf->Cell(25, 10, 'Register', 'BLR', 0, 'C');
        $pdf->Cell(75);
        $pdf->Cell(40, 10, 'Bapak Kandung', 'BLR', 0, 'C');
        $pdf->Cell(40, 10, 'Ibu Kandung', 'BLR', 0, 'C');
        $pdf->Ln(10);
        ...
        $pdf->Cell(70, 5, '(7)', 1, 0, 'C');
        $pdf->Cell(35, 5, '(8)', 1, 0, 'C');
    ...
    function Column2($pdf)
    {
        ...
        $pdf->Ln(10);
        $pdf->Cell(0, 10, 'SURAT KETERANGAN KELAHIRAN', 0, 0, 'C');
        $pdf->Ln(10);
    }

    public function LaporanKematian($id)
    {
//        array(215, 330)
        ...
        $this->Cover($pdf, $id);
        $pdf->AddPage();
        $pdf->SetTitle('Laporan Register Surat Kelahiran');
        ...
        $this->Column2($pdf);
        $this->Column($pdf, $id);
        $jumlah = $this->Kelahiran->getByPagecetak($id);
        ...
        $pdf->SetWidths([25, 25, 50, 25, 40, 40, 70, 35, 30, 40]);
        ...
        if ($jumlah == null) {
            ...
            foreach ($jumlah as $row) {
                ...
                if ($row->ibu_bayi == 1) {
                    $keluargaibu = $this->keluarga->cekalamat($row->ibu_penduduk_id);
                } else if ($row->bapak_bayi == 1) {
                    $keluargaibu = $this->keluarga->cekalamat($row->bapak_penduduk_id);
                }
                $alamatlengkap = $keluargaibu->alamat . ' RT. ' . $keluargaibu->alamat_rt . ' RW. ' . $keluargaibu->alamat_rw;

                $jk = $this->jk->find($row->jk_id);
                if ($row->ibu_bayi == 1) {
                    $pribadi1 = $this->pribadi->find($row->ibu_penduduk_id);
                    $namalengkapibu = $pribadi1->nama;
                }
                if ($row->ibu_bayi == 2) {
                    $pribadi1 = $this->nonpenduduk->find($row->ibu_penduduk_id);
                    $namalengkapibu = $pribadi1->nama;
                }
                if ($row->bapak_bayi == 1) {
                    $pribadi = $this->pribadi->find($row->bapak_penduduk_id);
                    $namalengkapbapak = $pribadi->nama;
                }
                if ($row->bapak_bayi == 2) {
                    $pribadi = $this->nonpenduduk->find($row->bapak_penduduk_id);
                    $namalengkapbapak = $pribadi->nama;
                }

                $pdf->Row([$row->tanggal, $row->no_reg, $row->nama, $jk->jk, $row->bapak_bayi_nik . ' ' . $namalengkapbapak, $row->ibu_bayi_nik . ' ' . $namalengkapibu, $alamatlengkap . ' ' . $statusdesa . ' ' . $namadesa . ' ' . $statuskecamatan . ' ' . $kecamatan . ' ' . $status . ' ' . $kabupaten, $namapejabatna, $users->name, '']);
...
//                $this->butuh = true;
            ...
            $tanggal = $this->Kelahiran->getByPagelimittanggal($id);
            if ($tanggal != null) {
