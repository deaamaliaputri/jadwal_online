...
class CetakLaporanSkck extends Controller
{
    protected $crud;
    protected $paginate;
//330 210
    ...
    function Cover($pdf, $id)
    {
        ...
        $kodeadministrasikearsipan = $this->Skck->cekkodejenisadministrasicetak();
        //kabupaten
        ...
        $pdf->Ln(10);
        $pdf->Cell(0, 5, 'SURAT KETERANGAN CATATAN KEPOLISIAN', 0, 0, 'C');
        $pdf->Ln(10);
...
//        echo $space_bottom . ' + ';
    ...
    function Column($pdf, $id)
    {
        ...
        $pdf->Cell(40, 20, 'Bapak Kandung', 1, 0, 'C');
        $pdf->Cell(40, 20, 'Ibu Kandung', 1, 0, 'C');
        $pdf->Cell(30, 20, 'Catatan OT', 1, 0, 'C');
    ...
    function Column2($pdf)
    {
        ...
        $pdf->Ln(10);
        $pdf->Cell(0, 10, 'SURAT KETERANGAN CATATAN KEPOLISIAN', 0, 0, 'C');
        $pdf->Ln(10);
    }

    public function LaporanSkck($id)
    {
//        array(215, 330)
        ...
        $this->Cover($pdf, $id);
        $pdf->AddPage();
        $pdf->SetTitle('Laporan Register Surat Skck');
        ...
        $this->Column2($pdf);
        $this->Column($pdf, $id);
        $jumlah = $this->Skck->getByPagecetak($id);
        $desa = $this->desa->find(session('desa'));
        ...
        $pdf->SetWidths([25, 20, 40, 25, 50, 40, 40, 30, 30, 30, 30, 30]);
        ...
        if ($jumlah == null) {
            ...
            foreach ($jumlah as $row) {
                ...
                if ($row->nik_bapak == '-') {
                    $cariorangtua = $this->orangtua->cekorangtuabapak($row->penduduk_id);

                    $namalengkapbapak = $cariorangtua->nama;
                }
                if ($row->nik_bapak != '-') {
                    if ($row->jenis_penduduk_bapak == '1') {
                        $pribadi1 = $this->pribadi->find($row->penduduk_bapak_id);

                        $namalengkapbapak = $pribadi1->nama;
                    }
                    if ($row->jenis_penduduk_bapak == '2') {
                        $nonpenduduk = $this->nonpenduduk->find($row->penduduk_bapak_id);
                        $namalengkapbapak = $nonpenduduk->nama;
                    }
                }
                if ($row->nik_ibu == '-') {
                    $cariorangtua = $this->orangtua->cekorangtuaibu($row->penduduk_id);

                    $namalengkapibu = $cariorangtua->nama;
                }
                if ($row->nik_ibu != '-') {
                    if ($row->jenis_penduduk_ibu == '1') {
                        $pribadi2 = $this->pribadi->find($row->penduduk_ibu_id);
                        $namalengkapibu = $pribadi2->nama;
                    }
                    if ($row->jenis_penduduk_ibu == '2') {
                        $nonpenduduk = $this->nonpenduduk->find($row->penduduk_ibu_id);
                        $namalengkapibu = $nonpenduduk->nama;
                    }
                }

                if ($row->jenis_catatan == '') {
                    $jeniscatatan = 'Tidak Terdaftar';
                }
                if ($row->jenis_catatan != '') {
                    if ($row->jenis_catatan == 'Terdaftar') {
                        $jeniscatatan = $row->uraian_catatan;

                    }
                    if ($row->jenis_catatan != 'Terdaftar') {
                        $jeniscatatan = $row->jenis_catatan;

                    }

                }
                $pdf->Row([$row->tanggal, $row->no_reg, $row->nik . $pribadi->titel_depan . ' ' . $pribadi->nama . ' ' . $pribadi->titel_belakang, $jk->jk, $keluarga->alamat . '' . ' RT. ' . $keluarga->alamat_rt . ' RW. ' . $keluarga->alamat_rw . ' ' . $statusdesa . ' ' . $namadesa . ' ' . $statuskecamatan . ' ' . $kecamatan . ' ' . $status . ' ' . $kabupaten, $namalengkapbapak, $namalengkapibu, $jeniscatatan, $row->penggunaan_surat, $namapejabatna, $users->name, '']);

                $tanggal = $this->Skck->getByPagelimittanggal($id);
                if ($tanggal != null) {
...
//        $pdf->set_line = false;
            ...
            $pdf->Output('cetak-data-register-Skck' . $tanggal . '.pdf', 'I');
            exit;
        }
    }
}