...
class CetakLaporanGoip extends Controller
{
    protected $crud;
    protected $paginate;
//330 210
    ...
    function Cover($pdf, $id)
    {
        ...
        $kodeadministrasikearsipan = $this->goip->cekkodejenisadministrasicetak();
        //kabupaten
        ...
        $pdf->Ln(10);
        $pdf->Cell(0, 5, 'SURAT KETERANGAN GHOIB', 0, 0, 'C');
        $pdf->Ln(10);
...
//        echo $space_bottom . ' + ';
    ...
    function Column($pdf, $id)
    {
        ...
        $pdf->Cell(25, 20, 'Jenis Kelamin', 1, 0, 'C');
        $pdf->Cell(70, 20, 'Alamat', 1, 0, 'C');
        $pdf->Cell(45, 20, 'Nama Termohon', 1, 0, 'C');
        $pdf->Cell(45, 20, 'Alasan', 'TLR', 0, 'C');
        ...
        $pdf->Cell(140);
        $pdf->Cell(45, 10, 'Gugatan', 'BLR', 0, 'C');
        $pdf->Ln(10);
    ...
    function Column2($pdf)
    {
        ...
        $pdf->Ln(10);
        $pdf->Cell(0, 10, 'SURAT KETERANGAN GHOIB', 0, 0, 'C');
        $pdf->Ln(10);
    }

    public function LaporanGoip($id)
    {
//        array(215, 330)
        ...
        $this->Cover($pdf, $id);
        $pdf->AddPage();
        $pdf->SetTitle('Laporan Register Surat Keterangan Ghoib');
        ...
        $this->Column2($pdf);
        $this->Column($pdf, $id);
        $jumlah = $this->goip->getByPagecetak($id);
        ...
        if ($jumlah != null) {
            ...
            foreach ($jumlah as $row) {
                ...
                if ($row->jenis_alasan_gugatan2 == 1 && $row->jenis_alasan_gugatan3 == 1 && $row->jenis_alasan_gugatan4 == 1 && $row->jenis_alasan_gugatan5 == 1) {
                    $alasangugatan = $row->jenis_alasan_gugatan1;
                }
                if ($row->jenis_alasan_gugatan2 != 1 && $row->jenis_alasan_gugatan3 == 1 && $row->jenis_alasan_gugatan4 == 1 && $row->jenis_alasan_gugatan5 == 1) {
                    $alasangugatan = $row->jenis_alasan_gugatan1 . ' ,' . $row->jenis_alasan_gugatan2;
                }
                if ($row->jenis_alasan_gugatan2 != 1 && $row->jenis_alasan_gugatan3 != 1 && $row->jenis_alasan_gugatan4 == 1 && $row->jenis_alasan_gugatan5 == 1) {
                    $alasangugatan = $row->jenis_alasan_gugatan1 . ' ,' . $row->jenis_alasan_gugatan2 . ' ,' . $row->jenis_alasan_gugatan3;
                }
                if ($row->jenis_alasan_gugatan2 != 1 && $row->jenis_alasan_gugatan3 != 1 && $row->jenis_alasan_gugatan4 != 1 && $row->jenis_alasan_gugatan5 == 1) {
                    $alasangugatan = $row->jenis_alasan_gugatan1 . ' ,' . $row->jenis_alasan_gugatan2 . ' ,' . $row->jenis_alasan_gugatan3 . ' ,' . $row->jenis_alasan_gugatan4;
                }
                if ($row->jenis_alasan_gugatan2 != 1 && $row->jenis_alasan_gugatan3 != 1 && $row->jenis_alasan_gugatan4 != 1 && $row->jenis_alasan_gugatan5 != 1) {
                    $alasangugatan = $row->jenis_alasan_gugatan1 . ' ,' . $row->jenis_alasan_gugatan2 . ' ,' . $row->jenis_alasan_gugatan3 . ' ,' . $row->jenis_alasan_gugatan4 . ' ,' . $row->jenis_alasan_gugatan5;
                }
                ...
                if ($row->pejabat_desa_id == 1) {
                    $keluarga = $this->keluarga->cekalamat($row->penduduk_id);
                    $pribadi = $this->pribadi->find($row->penduduk_id);

                    $jk = $this->jk->find($pribadi->jk_id);
//nama termohon
                    if ($row->terguguat_jenis_id == 1) {
                        $pribadi1 = $this->pribadi->find($row->terguguat_penduduk_id);
                        $namatergugat = $pribadi1->nama;
                    }
                    if ($row->terguguat_jenis_id == 2) {
                        $nonpenduduktergugat = $this->nonpenduduk->find($row->terguguat_penduduk_id);
                        $namatergugat = $nonpenduduktergugat->nama;
                    }
                    $pdf->Row([$row->tanggal, $row->no_reg, $row->nik . $pribadi->titel_depan . ' ' . $pribadi->nama . ' ' . $pribadi->titel_belakang, $jk->jk, $keluarga->alamat . '' . ' RT. ' . $keluarga->alamat_rt . ' RW. ' . $keluarga->alamat_rw . ' ' . $statusdesa . ' ' . $namadesa . ' ' . $statuskecamatan . ' ' . $kecamatan . ' ' . $status . ' ' . $kabupaten, $namatergugat, $alasangugatan, $row->penggunaan_surat, $namapejabatna, $users->name, '']);

                }
                if ($row->pejabat_desa_id == 2) {
                    $nonpenduduk = $this->nonpenduduk->find($row->penduduk_id);
                    $jk = $this->jk->find($nonpenduduk->jk_id);
                    $desanonpenduduk = $this->desa->find($nonpenduduk->desa_id);
...
//nama termohon
                    ...
                    //kabupaten
                    if ($desanonpenduduk->kecamatan->kabupaten->status == 1) {
                        $statuskabupatennonpenduduk = 'Kabupaten';
                        $kabupatennonpenduduk = $desanonpenduduk->kecamatan->kabupaten->kabupaten;
                    }
                    if ($desanonpenduduk->kecamatan->kabupaten->status == 2) {
                        $statuskabupatennonpenduduk = 'Kota';
                        $kabupatennonpenduduk = $desanonpenduduk->kecamatan->kabupaten->kabupaten;
                    }
                    //kecamatan
                    if ($desanonpenduduk->kecamatan->status == 1) {
                        $statuskecamatannonpenduduk = 'Kecamatan';
                        $kecamatannonpenduduk = $desanonpenduduk->kecamatan->kecamatan;
                    }
                    if ($desanonpenduduk->kecamatan->kabupaten->status == 2) {
                    ...
                    //desa
                    if ($desanonpenduduk->status == 1) {
                        $statusdesanonpenduduk = 'Kelurahan';
                        $namadesanonpenduduk = $desanonpenduduk->desa;
                    }
                    if ($desanonpenduduk->status == 2) {
                        $statusdesanonpenduduk = 'Desa';
                        $namadesanonpenduduk = $desanonpenduduk->desa;
                    }
                    if ($desanonpenduduk->status == 3) {
                        $statusdesanonpenduduk = 'Kampung';
                        $namadesanonpenduduk = $desanonpenduduk->desa;
                    }
                    if ($desanonpenduduk->status == 4) {
                        $statusdesanonpenduduk = 'Negeri';
                        $namadesanonpenduduk = $desanonpenduduk->desa;
                    }
                    if ($row->terguguat_jenis_id == 1) {
                        $pribadi2 = $this->pribadi->find($row->penduduk_id);
                        $namatergugat = $pribadi2->nama;
                    }
                    if ($row->terguguat_jenis_id == 2) {
                        $namatergugat = $nonpenduduk->nama;
                    }

                    $pdf->Row([$row->tanggal, $row->no_reg, $row->nik . $nonpenduduk->titel_depan . ' ' . $nonpenduduk->nama . ' ' . $nonpenduduk->titel_belakang, $jk->jk, $nonpenduduk->alamat . '|' . ' RT ' . $nonpenduduk->alamat_rt . '| RW ' . $nonpenduduk->alamat_rw . ' ' . $statusdesanonpenduduk . ' ' . $namadesanonpenduduk . ' ' . $statuskecamatannonpenduduk . ' ' . $kecamatannonpenduduk . ' ' . $statuskabupatennonpenduduk . ' ' . $kabupatennonpenduduk, $namatergugat, $alasangugatan, $row->penggunaan_surat, $namapejabatna, $users->name, '']);
...
//                $this->butuh = true;
            ...
            $tanggal = $this->goip->getByPagelimittanggal($id);
            if ($tanggal != null) {
...
//        $pdf->set_line = false;
        ...
        $pdf->Output('cetak-laporan--ghoib' . $tanggal . '.pdf', 'I');
        exit;
    }
}