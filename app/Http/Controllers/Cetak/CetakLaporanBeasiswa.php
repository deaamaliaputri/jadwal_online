...
class CetakLaporanBeasiswa extends Controller
{
    protected $crud;
    protected $paginate;
//330 210
    ...
    function Cover($pdf, $id)
    {
        ...
        $kodeadministrasikearsipan = $this->beasiswa->cekkodejenisadministrasicetak();
        //kabupaten
        ...
        $pdf->Ln(10);
        $pdf->Cell(0, 5, 'SURAT BEASISWA', 0, 0, 'C');
        $pdf->Ln(10);
...
//        echo $space_bottom . ' + ';
    ...
    function Column($pdf, $id)
    {
        ...
        $pdf->Cell(45, 20, 'Penerima Beasiswa', 1, 0, 'C');
        $pdf->Cell(45, 20, 'Informasi Pendidikan', 1, 0, 'C');
    ...
    function Column2($pdf)
    {
        ...
        $pdf->Ln(10);
        $pdf->Cell(0, 10, 'SURAT BEASISWA', 0, 0, 'C');
        $pdf->Ln(10);
    }

    public function LaporanBeasiswa($id)
    {
//        array(215, 330)
        ...
        $this->Cover($pdf, $id);
        $pdf->AddPage();
        $pdf->SetTitle('Laporan Register Surat Beasiswa');
        ...
        $this->Column2($pdf);
        $this->Column($pdf, $id);
        $jumlah = $this->beasiswa->getByPagecetak($id);
        ...
        if ($jumlah == null) {
            ...
            foreach ($jumlah as $row) {
                ...
                $pribadisiswa = $this->pribadi->find($row->pejabat_desa_id);

                $jk = $this->jk->find($pribadi->jk_id);
                $pdf->Row([$row->tanggal, $row->no_reg, $row->nik . $pribadi->titel_depan . ' ' . $pribadi->nama . ' ' . $pribadi->titel_belakang, $jk->jk, $keluarga->alamat . '|' . ' RT ' . $keluarga->alamat_rt . '| RW ' . $keluarga->alamat_rw . ' ' . $statusdesa . ' ' . $namadesa . ' ' . $statuskecamatan . ' ' . $kecamatan . ' ' . $status . ' ' . $kabupaten, $pribadisiswa->nama, $row->kelas . ' ' . $row->semester . ' ' . $row->sekolah_univ, $row->penggunaan, $namapejabatna, $users->name, '']);
...
//                $this->butuh = true;
            ...
            $tanggal = $this->beasiswa->getByPagelimittanggal($id);
            if ($tanggal != null) {
