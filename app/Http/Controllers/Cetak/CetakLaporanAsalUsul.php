...
class CetakLaporanAsalUsul extends Controller
{
    protected $crud;
    protected $paginate;
//330 210
    ...
    function Cover($pdf, $id)
    {
        ...
        $kodeadministrasikearsipan = $this->asalusul->cekkodejenisadministrasicetak();
        //kabupaten
...
//        echo $space_bottom . ' + ';
    ...
    function Column($pdf, $id)
    {
        ...
        $pdf->Cell(40, 20, 'Nama Bapak Kandung', 1, 0, 'C');
        $pdf->Cell(38, 20, 'Nama Ibu Kandung', 1, 0, 'C');
    ...
    public function LaporanAsalUsul($id)
    {
//        array(215, 330)
        ...
        $this->Column2($pdf);
        $this->Column($pdf, $id);
        $jumlah = $this->asalusul->getByPagecetak($id);
        ...
        if ($jumlah == null) {
            ...
            foreach ($jumlah as $row) {
                ...
                if ($row->pejabat_desa_id == 1) {
                    ...
                    $orangtuabapak1 = $this->orangtua->find($row->bapak);
                    $orangtuaibu1 = $this->orangtua->find($row->ibu);
                    $orangtuabapak = $orangtuabapak1->nama;
                    $orangtuaibu = $orangtuaibu1->nama;

                    $pdf->Row([$row->tanggal, $row->no_reg, $row->nik . $pribadi->titel_depan . ' ' . $pribadi->nama . ' ' . $pribadi->titel_belakang, $jk->jk, $keluarga->alamat . '' . ' RT. ' . $keluarga->alamat_rt . ' RW. ' . $keluarga->alamat_rw . ' ' . $statusdesa . ' ' . $namadesa . ' ' . $statuskecamatan . ' ' . $kecamatan . ' ' . $status . ' ' . $kabupaten, $orangtuabapak, $orangtuaibu, $row->penggunaan_surat, $namapejabatna, $users->name, '']);
