...
class CetakLaporanKeturunan extends Controller
{
    protected $crud;
    protected $paginate;
//330 210
    ...
    function Cover($pdf, $id)
    {
        ...
        $kodeadministrasikearsipan = $this->keturunan->cekkodejenisadministrasicetak();
        //kabupaten
        ...
        $pdf->Ln(10);
        $pdf->Cell(0, 5, 'SURAT KETERANGAN KETURUNAN', 0, 0, 'C');
        $pdf->Ln(10);
...
//        echo $space_bottom . ' + ';
    ...
    function Column($pdf, $id)
    {
        ...
        $pdf->Cell(70, 20, 'Alamat Domisli', 1, 0, 'C');
        $pdf->Cell(40, 20, 'Bapak/Ibu Kandung', 1, 0, 'C');
        $pdf->Cell(38, 20, 'Kakek/Nenek', 1, 0, 'C');
    ...
    function Column2($pdf)
    {
        ...
        $pdf->Ln(10);
        $pdf->Cell(0, 10, 'SURAT KETERANGAN KETURUNAN', 0, 0, 'C');
        $pdf->Ln(10);
    }

    public function LaporanKeturunan($id)
    {
//        array(215, 330)
        ...
        $this->Cover($pdf, $id);
        $pdf->AddPage();
        $pdf->SetTitle('Laporan Register Surat Keturunan');
        ...
        $this->Column2($pdf);
        $this->Column($pdf, $id);
        $jumlah = $this->keturunan->getByPagecetak($id);
        ...
        if ($jumlah == null) {
            ...
            foreach ($jumlah as $row) {
                ...
                if ($row->nama_kakek_bapak == 1) {
                    $namakakekbapakdanibu = '';
                }
                if ($row->nama_kakek_bapak != 1) {
                    $namakakekbapakdanibu = $row->nama_kakek_bapak . ' Dengan ' . $row->nama_nenek_bapak;
                }
                if ($row->nama_kakek_ibu == 1) {
                    $namakakekbapakdanibu1 = '';
                }
                if ($row->nama_kakek_ibu != 1) {
                    $namakakekbapakdanibu1 = ' dan ' . $row->nama_kakek_ibu . ' Dengan ' . $row->nama_nenek_ibu;
                }
                if ($row->jenis_penduduk == 1) {
                    ...
                    $orangtuabapak1 = $this->orangtua->cekorangtuabapak($row->penduduk_id);
                    $orangtuaibu1 = $this->orangtua->cekorangtuaibu($row->penduduk_id);
                    $orangtuabapak = $orangtuabapak1->nama;
                    $orangtuaibu = $orangtuaibu1->nama;

                    $pdf->Row([$row->tanggal, $row->no_reg, $row->nik . $pribadi->titel_depan . ' ' . $pribadi->nama . ' ' . $pribadi->titel_belakang, $jk->jk, $keluarga->alamat . '' . ' RT. ' . $keluarga->alamat_rt . ' RW. ' . $keluarga->alamat_rw . ' ' . $statusdesa . ' ' . $namadesa . ' ' . $statuskecamatan . ' ' . $kecamatan . ' ' . $status . ' ' . $kabupaten, $orangtuabapak . ' Dengan ' . $orangtuaibu, $namakakekbapakdanibu . $namakakekbapakdanibu1, $row->penggunaan_surat, $namapejabatna, $users->name, '']);

                }
                if ($row->jenis_penduduk == 2) {
                    $nonpenduduk = $this->nonpenduduk->find($row->penduduk_id);
                    $jk = $this->jk->find($nonpenduduk->jk_id);
                    $orangtuabapak = $row->nama_bapak;
                    $orangtuaibu = $row->nama_ibu;
...
//                $this->butuh = true;
            ...
            $tanggal = $this->keturunan->getByPagelimittanggal($id);
            if ($tanggal != null) {
