...
class CetakLaporanKehilangan extends Controller
{
    protected $crud;
    protected $paginate;
//330 210
    ...
    function Cover($pdf, $id)
    {
        ...
        $kodeadministrasikearsipan = $this->kehilangan->cekkodejenisadministrasicetak();
        //kabupaten
        ...
        $pdf->Ln(10);
        $pdf->Cell(0, 5, 'SURAT KETERANGAN KEHILANGAN DOKUMEN PENDUDUK', 0, 0, 'C');
        $pdf->Ln(10);
...
//        echo $space_bottom . ' + ';
    ...
    function Column($pdf, $id)
    {
        ...
        $pdf->Cell(70, 20, 'Alamat Domisili', 1, 0, 'C');
        $pdf->Cell(45, 20, 'Jenis Dokumen', 1, 0, 'C');
        $pdf->Cell(45, 20, 'Kronologi Kejadian', 1, 0, 'C');
    ...
    function Column2($pdf)
    {
        ...
        $pdf->Ln(10);
        $pdf->Cell(0, 10, 'SURAT KETERANGAN KEHILANGAN DOKUMEN PENDUDUK', 0, 0, 'C');
        $pdf->Ln(10);
    }

    public function LaporanKehilangan($id)
    {
//        array(215, 330)
        ...
        $this->Cover($pdf, $id);
        $pdf->AddPage();
        $pdf->SetTitle('Laporan Register Surat Kehilangan');
        ...
        $this->Column2($pdf);
        $this->Column($pdf, $id);
        $jumlah = $this->kehilangan->getByPagecetak($id);
        ...
        if ($jumlah == null) {
            ...
            foreach ($jumlah as $row) {
                ...
                if ($row->jenis_penduduk == 1) {
                    ...
                    $jk = $row->pribadi->jk->jk;
                    $namalengkap = $pribadi->titel_depan . ' ' . $pribadi->nama . ' ' . $pribadi->titel_belakang;
                ...
                if ($row->jenis_penduduk == 2) {
                    $jk = $row->non_penduduk->jk->jk;
                    $namalengkap = $row->non_penduduk->titel_depan . ' ' . $row->non_penduduk->nama . ' ' . $row->non_penduduk->titel_belakang;
                ...
                $pdf->Row([$row->tanggal, $row->no_reg, $row->nik . $namalengkap, $jk, $alamatlengkap, $row->jenis_dokumen . ' (' . $row->nomor_dokumen . ') ', $row->kronologi, $row->penggunaan, $namapejabatna, $users->name, '']);
...
//                $this->butuh = true;
            ...
            $tanggal = $this->kehilangan->getByPagelimittanggal($id);
            if ($tanggal != null) {
...
//        $pdf->set_line = false;
        ...
        $pdf->Output('cetak-data-register-kehilangan' . $tanggal . '.pdf', 'I');
        exit;
    }
}