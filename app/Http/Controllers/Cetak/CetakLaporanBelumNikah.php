...
class CetakLaporanBelumNikah extends Controller
{
    protected $crud;
    protected $paginate;
//330 210
    ...
    function Cover($pdf, $id)
    {
        ...
        $kodeadministrasikearsipan = $this->belumnikah->cekkodejenisadministrasicetak();
        //kabupaten
        ...
        $pdf->Ln(10);
        $pdf->Cell(0, 5, 'SURAT KETERANGAN BELUM NIKAH', 0, 0, 'C');
        $pdf->Ln(10);
...
//        echo $space_bottom . ' + ';
    ...
    function Column($pdf, $id)
    {
        ...
        $pdf->Cell(70, 20, 'Keterangan Tambahan', 1, 0, 'C');
        $pdf->Cell(50, 20, 'Penggunaan', 1, 0, 'C');
    ...
    function Column2($pdf)
    {
        ...
        $pdf->Ln(10);
        $pdf->Cell(0, 10, 'SURAT KETERANGAN BELUM NIKAH', 0, 0, 'C');
        $pdf->Ln(10);
    }

    public function LaporanBelumNikah($id)
    {
//        array(215, 330)
        ...
        $this->Cover($pdf, $id);
        $pdf->AddPage();
        $pdf->SetTitle('Laporan Register Surat Belum Nikah');
        ...
        $this->Column2($pdf);
        $this->Column($pdf, $id);
        $jumlah = $this->belumnikah->getByPagecetak($id);
        ...
        $pdf->SetWidths([25, 20, 40, 25, 70, 70, 50, 30, 30, 30]);
        ...
        if ($jumlah == null) {
            ...
            foreach ($jumlah as $row) {
                ...
                $pdf->Row([$row->tanggal, $row->no_reg, $row->nik . $pribadi->titel_depan . ' ' . $pribadi->nama . ' ' . $pribadi->titel_belakang, $jk->jk, $keluarga->alamat . '' . ' RT ' . $keluarga->alamat_rt . ' RW ' . $keluarga->alamat_rw . ' ' . $statusdesa . ' ' . $namadesa . ' ' . $statuskecamatan . ' ' . $kecamatan . ' ' . $status . ' ' . $kabupaten,  $row->keterangan_tambahan, $row->penggunaan_surat, $namapejabatna, $users->name, '']);
...
//                $this->butuh = true;
            ...
            $tanggal = $this->belumnikah->getByPagelimittanggal($id);
            if ($tanggal != null) {
