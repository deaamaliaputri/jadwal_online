...
class CetakModelBepergian extends Controller
{
    protected $crud;
    protected $paginate;
//330 210
    ...
    function Cover($pdf, $id)
    {
        ...
        $kodeadministrasikearsipan = $this->bepergian->cekkodejenisadministrasicetak();
        //kabupaten
        ...
        $pdf->Ln(10);
        $pdf->Cell(0, 5, 'SURAT KETERANGAN BEPERGIAN', 0, 0, 'C');
        $pdf->Ln(10);
...
//        echo $space_bottom . ' + ';
    ...
    function Column($pdf, $id)
    {
        ...
        $pdf->Cell(20, 20, 'L/P', 1, 0, 'C');
        $pdf->Cell(60, 20, 'Alamat Domisili Asal', 1, 0, 'C');
        $pdf->Cell(60, 20, 'Alamat Tujuan Bepergian', 1, 0, 'C');
        $pdf->Cell(40, 20, 'NIK & Nama Lengkap', 1, 0, 'C');
        $pdf->Cell(30, 20, 'Alasan Bepergian', 1, 0, 'C');
        $pdf->Cell(25, 20, 'Waktu Bepergian', 1, 0, 'C');
        ...
        $pdf->Ln(5);
        $pdf->Cell(40);
        $pdf->Cell(35, 13, 'Nama Pemohon', 0, 0, 'C');

        $pdf->Ln(5);
        $pdf->Cell(20);
        $pdf->Cell(20, 10, 'Register', 'BLR', 0, 'C');
        $pdf->Cell(175);
        $pdf->Cell(40, 10, 'Pengikut', 'BLR', 0, 'C');
        $pdf->Ln(10);
    ...
    function Column2($pdf)
    {
        ...
        $pdf->Ln(10);
        $pdf->Cell(0, 10, 'SURAT KETERANGAN BEPERGIAN', 0, 0, 'C');
        $pdf->Ln(10);
    }

    public function ModelBepergian($id)
    {
//        array(215, 330)
        ...
        $this->Cover($pdf, $id);
        $pdf->AddPage();
        $pdf->SetTitle('Laporan Model Bepergian');
        ...
        $this->Column2($pdf);
        $this->Column($pdf, $id);
        $jumlah = $this->bepergian->getByPagecetak($id);
        ...
        $pdf->SetWidths([20, 20, 35, 20, 60, 60, 40, 30, 25, 30, 30, 30]);
        ...
        if ($jumlah == null) {
            ...
            foreach ($jumlah as $row) {
                ...
                $sampaihari = $this->bepergian->ceksampaikapan($row->id);
                $namapejabat = $this->pejabat->cekjabatan($row->penandatangan);

                //desa->tujuan
                //kabupaten
                if ($row->desa_tujuan_id->kecamatan->kabupaten->status == 1) {
                    $statustujuan = 'Kabupaten';
                    $kabupatentujuan = $row->desa_tujuan_id->kecamatan->kabupaten->kabupaten;
                }
                if ($row->desa_tujuan_id->kecamatan->kabupaten->status == 2) {
                    $statustujuan = 'Kota';
                    $kabupatentujuan = $row->desa_tujuan_id->kecamatan->kabupaten->kabupaten;
                }
                //kecamatan
                if ($row->desa_tujuan_id->kecamatan->status == 1) {

                    $statuskecamatantujuan = 'Kecamatan';
                    $kecamatantujuan = $row->desa_tujuan_id->kecamatan->kecamatan;
                }
                if ($row->desa_tujuan_id->kecamatan->kabupaten->status == 2) {
                ...
                //desa
                if ($row->desa_tujuan_id->status == 1) {
                    $statusdesatujuan = 'Kelurahan';
                    $namadesatujuan = $row->desa_tujuan_id->desa;
                }
                if ($row->desa_tujuan_id->status == 2) {

                    $statusdesatujuan = 'Desa';
                    $namadesatujuan = $row->desa_tujuan_id->desa;
                }
                if ($row->desa_tujuan_id->status == 3) {

                    $statusdesatujuan = 'Kampung';
                    $namadesatujuan = $row->desa_tujuan_id->desa;
                }
                if ($row->desa_tujuan_id->status == 4) {

                    $statusdesatujuan = 'Negeri';
                    $namadesatujuan = $row->desa_tujuan_id->desa;
                }
                if ($row->pengikut_2 != 1 && $row->pengikut_3 != 1 && $row->pengikut_4 != 1 && $row->pengikut_5 != 1) {
                    $pribadi = $this->pribadi->ceknik($row->pengikut_1);
                    $pribadi2 = $this->pribadi->ceknik($row->pengikut_2);
                    $pribadi3 = $this->pribadi->ceknik($row->pengikut_3);
                    $pribadi4 = $this->pribadi->ceknik($row->pengikut_4);
                    $pribadi5 = $this->pribadi->ceknik($row->pengikut_5);

                    $pengikut = '( ' . $row->pengikut_1 . ' ' . $pribadi->nama . ' ), ' . '( ' . $row->pengikut_2 . ' ' . $pribadi2->nama . ' ), ' . '( ' . $row->pengikut_3 . ' ' . $pribadi3->nama . ' ), ' . '( ' . $row->pengikut_4 . ' ' . $pribadi4->nama . ' ), ' . '(' . $row->pengikut_5 . ' ' . $pribadi5->nama . ' )';
                }
                if ($row->pengikut_2 != 1 && $row->pengikut_3 != 1 && $row->pengikut_4 != 1 && $row->pengikut_5 == 1) {
                    $pribadi = $this->pribadi->ceknik($row->pengikut_1);
                    $pribadi2 = $this->pribadi->ceknik($row->pengikut_2);
                    ...
                    $pengikut = '( ' . $row->pengikut_1 . ' ' . $pribadi->nama . ' ), ' . '( ' . $row->pengikut_2 . ' ' . $pribadi2->nama . ' ), ' . '( ' . $row->pengikut_3 . ' ' . $pribadi3->nama . ' ), ' . '( ' . $row->pengikut_4 . ' ' . $pribadi4->nama . ' )';
                }
                if ($row->pengikut_2 != 1 && $row->pengikut_3 != 1 && $row->pengikut_4 == 1 && $row->pengikut_5 == 1) {
                    $pribadi = $this->pribadi->ceknik($row->pengikut_1);
                    $pribadi2 = $this->pribadi->ceknik($row->pengikut_2);
                    ...
                    $pengikut = '( ' . $row->pengikut_1 . ' ' . $pribadi->nama . ' ), ' . '( ' . $row->pengikut_2 . ' ' . $pribadi2->nama . ' ), ' . '( ' . $row->pengikut_3 . ' ' . $pribadi3->nama . ' )';
                }
                if ($row->pengikut_2 != 1 && $row->pengikut_3 == 1 && $row->pengikut_4 == 1 && $row->pengikut_5 == 1) {
                    $pribadi = $this->pribadi->ceknik($row->pengikut_1);
                    $pribadi2 = $this->pribadi->ceknik($row->pengikut_2);

                    $pengikut = '( ' . $row->pengikut_1 . ' ' . $pribadi->nama . ' ), ' . '( ' . $row->pengikut_2 . ' ' . $pribadi2->nama . ' )';
                }
                if ($row->pengikut_2 == 1 && $row->pengikut_3 == 1 && $row->pengikut_4 == 1 && $row->pengikut_5 == 1) {
                    $pribadi = $this->pribadi->ceknik($row->pengikut_1);
                    $pengikut = '( ' . $row->pengikut_1 . ' ' . $pribadi->nama . ' )';
                }
                if ($row->pengikut_1 == 1 && $row->pengikut_2 == 1 && $row->pengikut_3 == 1 && $row->pengikut_4 == 1 && $row->pengikut_5 == 1) {
                    $pengikut = '';
                }
                ...
                $pdf->Row([$row->tanggal, $row->no_reg, $row->nik . $pribadi->titel_depan . ' ' . $pribadi->nama . ' ' . $pribadi->titel_belakang, $jk->jk, $keluarga->alamat . '' . ' RT. ' . $keluarga->alamat_rt . ' RW. ' . $keluarga->alamat_rw . ' ' . $statusdesa . ' ' . $namadesa . ' ' . $statuskecamatan . ' ' . $kecamatan . ' ' . $status . ' ' . $kabupaten, $row->alamat . ' RT. ' . $row->alamat_rt . ' RW. ' . $row->alamat_rw . ' ' . $statusdesatujuan . ' ' . $namadesatujuan . ' ' . $statuskecamatantujuan . ' ' . $kecamatantujuan . ' ' . $statustujuan . ' ' . $kabupatentujuan, $pengikut, $alasan, $sampaihari . ' ', $namapejabatna, $users->name, '']);
...
//                $this->butuh = true;
            ...
            $tanggal = $this->bepergian->getByPagelimittanggal($id);
            if ($tanggal != null) {
